...
| "Save routes for offline" is granted, and nothing is saved | **A real defect, open, and the UI claims otherwise.** `sw.js:186` returns early for any cross-origin request — the line that keeps map tiles from ever being cached, and it is right — but this deployment serves the site from `meander-eoc.pages.dev` and the API from `meander-app.duckdns.org`, so *every* API call is cross-origin and the `/api/routes` branch below it never runs. Measured live: consent granted, search completed, zero results caches, nothing stored. BLOCKED.md §8 has the three ways out and why the choice belongs to the operator. |
